...
# LTOOLS 本地分类入口使用；为空时保持上游原始交互菜单行为。
LOCAL_ACTION=""
...
Documentation=https://github.com/LYISTR2/LTOOLS/tree/main/vps-tcp-tune-local
...
# LTOOLS 本地化单次动作分发。
# 功能函数仍集中在共享核心中，分类入口只传递动作编号，避免重复代码和依赖断裂。
run_local_action() {
    local choice="$1"
    ...
    case "$choice" in
        ...
        2)
            ...
            ;;
        3) bbr_configure_direct; break_end ;;
        5) dns_purify_and_harden ;;
        6) realm_fix_timeout; break_end ;;
        7) manage_ip_priority ;;
        8) manage_ipv6 ;;
        9) set_temp_socks5_proxy ;;
        10) manage_swap ;;
        11) show_detailed_status ;;
        12) snell_menu ;;
        13) run_xinchendahai_xray ;;
        14) manage_cn_ip_block ;;
        15) manage_socks5 ;;
        16) manage_substore ;;
        17) manage_reverse_proxy ;;
        18) run_ip_quality_check ;;
        19) run_ip_quality_check_ipv4 ;;
        20) run_speedtest ;;
        21) iperf3_single_thread_test ;;
        22) run_international_speed_test ;;
        23) run_network_latency_check ;;
        24) run_backtrace ;;
        25) run_unlock_check ;;
        26) run_ns_detect ;;
        27) run_pf_realm ;;
        28) run_fscarmen_singbox ;;
        29) run_kejilion_script ;;
        30) startbbrcake ;;
        31) Kernel_optimize ;;
        32) ai_proxy_menu ;;
        33) ptm_menu ;;
