...
| 4 | VPS 综合质量体检 | `https://run.NodeQuality.com` |
| 5 | VPS节点搭建 | 本地安装的 `/usr/local/bin/sb`（singbox-lite） |
| 0 | 退出 | — |
...
首次选择菜单 `5` 时，LTOOLS 会从 `0xdabiaoge/singbox-lite` 下载 `singbox.sh`，完成 HTTPS 下载、HTML 响应拦截、Bash 语法检查和 SHA-256 显示后，安装为：

```text
/usr/local/bin/sb
```

以后再次选择菜单 `5`，只会运行这份本地脚本，不会重新远程下载。也可以在终端直接调用：

```bash
sudo sb
```

该上游工具要求 root 权限。需要更新时，可进入 `sb` 自身菜单并选择 `13. 更新脚本`；如果它被卸载，下一次选择 LTOOLS 菜单 `5` 会自动重新部署。
